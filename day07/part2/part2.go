...
func SearchDirToDelete(dirMap map[string]int){
	totalSize := 70000000
	rootSize := dirMap["/"]
	sizeRemaing := totalSize - rootSize
	sizeWant := 30000000 - sizeRemaing
	fmt.Println(sizeRemaing)
	fmt.Println(sizeWant)
	var sizeGreatList []map[string]int

	for key, val := range dirMap{
		if val >= sizeWant{
			sizeGreatList = append(sizeGreatList, map[string]int{key: val})
		}
	}
	fmt.Println(sizeGreatList)
	min := 100000000000
	var ans int
	for _, myMap := range sizeGreatList{
